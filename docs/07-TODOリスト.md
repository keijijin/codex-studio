...
| 版 | 1.0 |
| 作成日 | 2026-07-15 |
| 更新日 | 2026-07-18（Shell 環境同期・細かい機能メモ追加） |
...
## Phase R: マルチモデルスイングリング（自動ルーティング）

> 詳細: [10-マルチモデルルーティングプラン.md](./10-マルチモデルルーティングプラン.md)

### R1 — フォールバック基盤

- [x] `routing` 設定スキーマ（`mode` / `fallbackChain` / `maxAttempts`）
- [x] 設定マイグレーション（既存ユーザーは `fixed` 既定）
- [x] `ModelRouter` 骨格（fixed / fallback-only）
- [x] 再試行可能エラー判定（429 / 5xx / 接続失敗。401 は除外）
- [x] API キー未設定・Ollama 未達候補のスキップ
- [x] Chat サービスへのフォールバック組込（二重投稿防止）
- [x] Agent サービスへのフォールバック組込（**ツール開始前のみ**）
- [x] CLI でのフォールバック組込
- [x] R1 単体・統合テスト（mock provider）

### R2 — ヒューリスティック自動選択

- [x] `classifyTaskKind`（chat_simple / agent_code / team 等・LLM なし）
- [x] タスク種別ごとの候補プロファイル表（設定で上書き可）
- [x] 利用可能性フィルタ（キー有無。Ollama ping は R4）
- [x] `routing.mode = auto` で Chat / Agent / CLI 統一解決
- [x] R2 スナップショット／単体テスト

### R3 — UI / CLI / 透明性

- [x] 設定 UI：Routing mode 選択 + fallback チェーン詳細編集
- [x] AIPanel：Fixed / Fallback / Auto クイック切替
- [x] 選択結果の表示（モデル + reason）
- [x] Stream イベント：再試行中・切替通知
- [x] CLI：`--routing` と json 出力の reason
- [x] 操作マニュアル更新
...
| **Phase R ルーティング** | **R1–R3 完了** | クイック切替・チェーン編集込み。R4 は任意 |
| **Shell 環境連携** | **完了** | プロファイル自動読込 + ターミナル同期 + `agent.env` |

---

## 今週のフォーカス

1. Shell 環境同期と Phase R の dogfooding
2. α 安定化
...
| **2026-07-18** | **マルチモデル自動ルーティング方針を決定（プラン 10）** |
| | **既定は fixed。auto / fallback-only はオプトイン** |
| | **ツール実行開始後のフォールバックは行わない（副作用回避）** |
| | **順序: R1 フォールバック → R2 ヒューリスティック → R3 UI → R4 高度化** |
| **2026-07-18** | **Agent Shell のログインプロファイル自動読込を実装** |
| | **ターミナル環境同期 + `.codex/agent.env`（文書: [11](./11-シェル環境と細かい機能メモ.md)）** |
