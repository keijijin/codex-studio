...
3. 設定から API キー（OpenAI / Anthropic / xAI Grok）または Ollama を設定
4. 右側 **AI Chat** パネルで質問
...
| CLI が Ollama に落ちる（`-p xai` なのに） | 環境変数未設定でもアプリ設定のキーを使います。まだなら設定画面に xAI キーを保存するか `export XAI_API_KEY=...` |
| CLI が `--use-system-ca is not allowed in NODE_OPTIONS` | 統合ターミナルが Electron 用フラグを引き継いだ場合。`unset NODE_OPTIONS` するか、最新の `pnpm cli`（自動除去）を使う |
